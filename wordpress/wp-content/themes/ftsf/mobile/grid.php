<?php
require_once(dirname(__FILE__) . '/../grid-manager/database-manager/db-manager.php');
require_once(dirname(__FILE__) . '/../grid-manager/grid-generation/generate-block.php');
...
foreach($grid as $position){
    if( $current_column != $position->get_column()->get_column() ){
        if( $pattern_id != null ){
            require_once(dirname(__FILE__) . "/../grid-manager/grid-generation/patterns/pattern$pattern_id.php");
